<template>
    <div class="col-md-3">
        <div class="card text-white bg-primary col-md-12 items" style="max-width: 18rem;">
            ...
            <div class="card-body">
                <div class="card standard-card text-white bg-dark mb-3" style="max-width: 18rem;" v-for="(kanban,i) in doing" v-bind:key="i" v-if="kanban.status == 'doing'">
                    ...
                    <div class="card-footer bg-transparent border-light" style="padding: 0px; padding-top: 17px">
                        <button type="button" class="btn btn-outline-light" data-toggle="modal" @click="getTask(i)" data-target="#modalDoing">Show Detail</button>
                    </div>
                </div>
            </div>
        </div>
        <!-- Modal -->
        <div class="modal fade" id="modalDoing" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer" style="justify-content: center;">
                        <button type="button" class="btn btn-warning" @click="previous(task)" data-dismiss="modal">Todo</button>
                        <button type="button" class="btn btn-danger" @click="deleteTask" data-dismiss="modal">Delete</button>
                        <button type="button" class="btn btn-success" @click="next(task)" data-dismiss="modal">Approved</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(['getAllTask', 'getOneTask', 'updateToDo', 'updateApproved', 'removeTask']),
            getTask(id) {
